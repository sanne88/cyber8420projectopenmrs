...
Furthermore, to mitigate the risk of dictionary attacks on commonly used passwords, the system implements a stringent password policy, ensuring that users create strong and unique passwords. To fortify the password update process, a secret question is employed to validate the user's identity. Currently, one secret question is utilized, but it is advisable to incorporate multiple questions for added security.
